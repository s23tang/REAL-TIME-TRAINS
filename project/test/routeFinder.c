...
void route( track_node *track, Path *shortest, int src, int dest, int sender, Sensor *sensors, Branch *branches, int myAdmin ) {
	int dist[TRACK_MAX];
	Loc prev[TRACK_MAX];
	int Q[TRACK_MAX];
	...
	while ( num_left != 0 ) {
		...
		if ( lowestDist == dest || lowestDist == -1 ) break;
		Q[lowestDist] = -1;
		num_left--;
		int alt, v, num, count;
		node_type type;
		...
		for ( i = 0; i < count; i++ ) {
			if ( lowestDist < 80 ) {
				if ( sensors[(track[lowestDist].reverse)->num].reserved == 1 && sensors[(track[lowestDist].reverse)->num].reservedBy != sender ) {
					break;
				}
			} else {
				if ( lowestDist % 2 == 0 ) {
					if ( branches[lowestDist].reserved == 1 && branches[lowestDist].reservedBy != sender ) {
						break;
					}
				} else {
					if ( branches[lowestDist-1].reserved == 1 && branches[lowestDist-1].reservedBy != sender ) {
						break;
					}
				}
			}
			if ( lowestDist == 100 && i == 1 ) break;
...
void routeFinder( ) {
	...
	send.type = ARRAY_ADDRESS;
	Send(monitor, (char *)&send, sizeof(ComReqStruct), (char *)&reply, sizeof(ComReqStruct) );

	Sensor *sensors = (Sensor *)reply.data1;
	Branch *branches = (Sensor *)reply.data2;
	...
	FOREVER {
		...
		route( track, &paths[i], src, dest, sender, sensors, branches, myAdmin );
