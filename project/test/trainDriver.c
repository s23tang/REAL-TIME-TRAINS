...
void trainDriver()
{
    ...
    FOREVER
    {
        ...
        Receive( &sender, (char *)&reply, sizeof(ComReqStruct) );
        int thisTrain = reply.data1;
        if (reply.type == GOTO_COMMAND)
        {
	...
	GET_ROUTE:
        ...
        }else if (reply.type == CATCH_COMMAND){    // catch commmand: for final demo
            Reply( sender, (char *)&send, sizeof(ComReqStruct) );   // <--- unblocking terminal
...
;
        	...
        	{
				send.type  = SENSOR_SUBSCRIBE;
				send.data1 = 4; // not sure where i am. No guess for now
				send.data2 = -1;
				Send( monitor, (char *)&send, sizeof(ComReqStruct), (char *)&reply, sizeof(ComReqStruct) );

	            send.type = SPEED_COMMAND;
	            send.data1 = 10;
	            send.data2 = thisTrain;
	            Send( myAdmin, (char *)&send, sizeof(ComReqStruct), (char *)&reply, sizeof(ComReqStruct));
        	}
            ...
            FOREVER {
				...
				if (reply.type == SWAP)
				{
		            ...
		            // acquire the current location of the target train
		            send.type = TARGET_LOC;
		            send.data1 = 1;
		            Send( monitor, (char *)&send, sizeof(ComReqStruct), (char *)&reply, sizeof(ComReqStruct));
		            targetLoc = reply.data1;
		            myLoc = reply.data2;

		            if (myLoc == targetLoc)
		            {	// should set the speed and break; 
	    	            send.type = SPEED_COMMAND;
			            send.data1 = 6;
			            send.data2 = thisTrain;
			            Send( myAdmin, (char *)&send, sizeof(ComReqStruct), (char *)&reply, sizeof(ComReqStruct));
        ...
        } else if(reply.type == KEEP_RUNNING) {
            Reply( sender, (char *)&send, sizeof(ComReqStruct) );   // <--- unblocking terminal
KEEPRUNNING:
;
			int myLoc = -1;

            int sensorsInLoop[12];
            sensorsInLoop[0] = 31;
            sensorsInLoop[1] = 41;
            sensorsInLoop[2] = 16;
            sensorsInLoop[3] = 61;
            ...
            {
				send.type  = SENSOR_SUBSCRIBE;
				send.data1 = sensorsInLoop[i];
				send.data2 = -1;
				Send( monitor, (char *)&send, sizeof(ComReqStruct), (char *)&reply, sizeof(ComReqStruct) );

	            // Move the train
				send.type  = SPEED_COMMAND;
				send.data1 = 6;      // Start with a slow speed, until triger a sensor        
				send.data2 = thisTrain;
				Send(myAdmin, (char*)&send, sizeof(ComReqStruct), (char*)&reply, sizeof(reply));
			...
			FOREVER{
				...
				if (reply.type == SWAP)
				{
					...
					send.type  = UPDATE_LOCATION;
					send.data1 = sensorsInLoop[i]; // not sure where i am. No guess for now
					send.data2 = -1;
					Send( monitor, (char *)&send, sizeof(ComReqStruct), (char *)&reply, sizeof(ComReqStruct) );
					i = (i+1)%12;
				}
